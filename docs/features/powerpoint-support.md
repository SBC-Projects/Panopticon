...
- `2026-05-27` — Background slide cache: PNGs under `data/pptx-cache/<id>/` are pre-warmed on file add/change (watcher + boot scan) via a serialized COM queue in `pptx-render.ts`. While a newer version renders, the API serves the previous PNG set (stale-while-revalidate); renders land in `.rendering/` then promote atomically.
- `2026-05-27` — Stale-cache UX: preview JSON carries `slides_cache` (`fresh` | `stale`) and `slides_cache_key` for PNG cache-busting. `DocPreview` shows “Changes detected — re-rendering slides…” and polls until fresh.
